...
import { UseContentInterface, UseContentErrors } from '~/_old/composables/useContent/useContent';
...
export const useContent = <PAGE, BLOCK>(): UseContentInterface<PAGE, BLOCK> => {
  ...
  const loadPage = async (params: ComposableFunctionArgs<{ identifier: string }>): Promise<PAGE> => {
    ...
    try {
      ...
      const { loadContentCommand } = await import('~/_old/composables/useContent/commands/loadContentCommand');
