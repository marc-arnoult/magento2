...
import { UseCartInterface } from '~/_old/composables/useCart/useCart';
import { useCustomerStore } from '~/_old/stores/customer';
...
(): UseCartInterface<CART, CART_ITEM, PRODUCT> => {
  ...
  const load = async ({ customQuery = {}, realCart = false } = { customQuery: { cart: 'cart' } }): Promise<void> => {
    ...
    try {
      loading.value = true;
      const { loadCartCommand } = await import('~/_old/composables/useCart/commands/loadCartCommand');
  ...
  const clear = async ({ customQuery } = { customQuery: { cart: 'cart' } }): Promise<void> => {
    ...
    try {
      ...
      const [{ loadCartCommand }, { clearCartCommand }] = await Promise.all(
        [
          import('~/_old/composables/useCart/commands/loadCartCommand'),
          import('~/_old/composables/useCart/commands/clearCartCommand'),
        ],
      );
  ...
  const addItem = async ({
    ...
    try {
      ...
      const { addItemCommand } = await import('~/_old/composables/useCart/commands/addItemCommand');
  ...
  const removeItem = async ({ product }) => {
    ...
    try {
      loading.value = true;
      const { removeItemCommand } = await import('~/_old/composables/useCart/commands/removeItemCommand');
  ...
  const updateItemQty = async ({
    ...
    if (quantity && quantity > 0) {
      try {
        loading.value = true;
        const { updateItemQtyCommand } = await import('~/_old/composables/useCart/commands/updateItemQtyCommand');
  ...
  const applyCoupon = async ({
    ...
    try {
      loading.value = true;
      const { applyCouponCommand } = await import('~/_old/composables/useCart/commands/applyCouponCommand');
  ...
  const removeCoupon = async ({
    ...
    try {
      loading.value = true;
      const { removeCouponCommand } = await import('~/_old/composables/useCart/commands/removeCouponCommand');
